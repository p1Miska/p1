...
public class WindowInHandRenderer {
	...
	public void render(PoseStack poseStack, SubmitNodeCollector collector, float attack, float handHeight, int light, HumanoidArm humanoidArm, ItemStack itemStack) {
		...
		render(poseStack, collector, attack, handHeight, light, humanoidArm, toplevel);
	}
	
	// Phantom-friendly overload: renders a live toplevel in hand without
	// needing a real ItemStack at all.
	public void render(PoseStack poseStack, SubmitNodeCollector collector, float attack, float handHeight, int light, HumanoidArm humanoidArm, WLCToplevel toplevel) {
		poseStack.pushPose();
		...
		renderWindow(poseStack, collector, h, light, toplevel);
		
		poseStack.popPose();
	}
	
	public void renderWindow(PoseStack poseStack, SubmitNodeCollector collector, float sideMult, int light, ItemStack itemStack) {
		renderWindow(poseStack, collector, sideMult, light, WaylandCraft.getToplevel(itemStack));
	}
	
	public void renderWindow(PoseStack poseStack, SubmitNodeCollector collector, float sideMult, int light, WLCToplevel toplevel) {
