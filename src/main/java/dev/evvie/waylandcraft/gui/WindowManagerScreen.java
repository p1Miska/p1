...
public class WindowManagerScreen extends Screen {
	...
	private Button[] phantomSlotButtons = new Button[dev.evvie.waylandcraft.item.PhantomWindowSlots.SLOT_COUNT];
	...
	protected void init() {
		...
		for(int i = 0; i < phantomSlotButtons.length; i++) {
			final int slot = i;
			Button btn = Button.builder(Component.literal("P" + (slot + 1)), (b) -> onPhantomSlotPressed(slot))
					.size(22, 22)
					.build();
			btn.setPosition(3, topMargin + 90 + slot * 24);
			btn.setTooltip(Tooltip.create(Component.literal("Assign to phantom slot " + (slot + 1) + " (no server item, bind a key to use it)")));
			btn.setTooltipDelay(Duration.ofMillis(700));
			phantomSlotButtons[slot] = btn;
			buttons.add(btn);
		}
		...
		for(Button btn : phantomSlotButtons) addRenderableWidget(btn);
	...
	// Assigns the currently focused toplevel to a phantom slot.
	// No network packet, no server-side ItemStack — purely local state.
	private void onPhantomSlotPressed(int slot) {
		if(focused == null) return;
		wlc.phantomSlots.set(slot, focused);
	}
