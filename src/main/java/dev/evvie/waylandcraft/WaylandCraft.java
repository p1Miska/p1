...
import dev.evvie.waylandcraft.item.PhantomWindowSlots;
...
public class WaylandCraft implements ClientModInitializer {
	...
	public KeyMapping[] keyPhantomSlots = new KeyMapping[PhantomWindowSlots.SLOT_COUNT];
	...
	// Phantom (client-only, no server item) window grab state
	public PhantomWindowSlots phantomSlots = new PhantomWindowSlots(this);
	private WLCToplevel phantomGrabToplevel = null;
	private boolean playerUsingPhantomWindow = false;
	private boolean playerWasUsingPhantomWindow = false;
	...
	public void onInitializeClient() {
		...
		for(int i = 0; i < keyPhantomSlots.length; i++) {
			int slot = i;
			keyPhantomSlots[slot] = KeyMappingHelper.registerKeyMapping(new KeyMapping("waylandcraft.key.phantomSlot." + (slot + 1), InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_UNKNOWN, KEYBIND_CATEGORY));
		}
	...
	public void updateWorld(LevelExtractionContext ctx) {
		...
		// Phantom (no real item, no server) window grab, driven by held keybinds instead of getUseItem()
		boolean anyPhantomKeyDown = false;
		for(int i = 0; i < keyPhantomSlots.length; i++) {
			if(keyPhantomSlots[i].isDown()) {
				phantomSlots.useTick(i);
				anyPhantomKeyDown = true;
			}
		}
		if(!anyPhantomKeyDown) {
			playerUsingPhantomWindow = false;
			phantomGrabToplevel = null;
		}
		if(playerUsingPhantomWindow && phantomGrabToplevel != null) {
			WindowDisplay display = getOrCreateDisplay(phantomGrabToplevel);
			if(!playerWasUsingPhantomWindow) {
				display.anchorDistance = 2.0;
			}
			
			display.doGrabMove(camera.position(), new Vec3(camera.forwardVector()), new Vec3(camera.upVector()), camera.yRot());
			
			WaylandCraft.instance.bridge.focusSurface(phantomGrabToplevel);
		}
		playerWasUsingPhantomWindow = playerUsingPhantomWindow;
	...
	// Called every tick by PhantomWindowSlots while its keybind is held
	public void startUsingPhantomWindow(WLCToplevel toplevel) {
		playerUsingPhantomWindow = true;
		phantomGrabToplevel = toplevel;
	}
	
	// Used by ItemInHandRendererMixin to render a phantom window live in the
	// offhand, even though no real ItemStack exists for it.
	public WLCToplevel getPhantomOffhandToplevel() {
		if(!playerUsingPhantomWindow) return null;
		return phantomGrabToplevel;
	}
	...
	public boolean onScroll(long windowHandle, double scrollX, double scrollY) {
		...
		if(playerUsingPhantomWindow && phantomGrabToplevel != null) {
			WindowDisplay display = getDisplay(phantomGrabToplevel);
			if(display != null) {
				display.adjustAnchorDistance(scrollY);
				return true;
			}
